...
export function WeekView() {
  ...
  // S25 — drag-to-reschedule. A SINGLE DndContext wraps both the `md:+`
  // 7-column grid AND the `< md` agenda (the agenda renders inside WeekView),
  // so a chip dragged in either layout resolves against the same droppable
  // ids (the day `date`).
    ...
    <section data-testid="week-view" className="border-border overflow-hidden rounded-md border">
      ...
      {query.data && (
        ...
        >
              ...
              {days.map((day, idx) => {
                ...
                return (
                  <WeekDayColumn
                    ...
                    date={date}
                    isToday={isToday}
                    label={weekdayHeaders[idx] ?? ''}
                    shortDate={formatDate(date).slice(0, 5)}
                    onDayClick={flow.handleDayClick}
                  >
          ...
          {/* Drag clone follows the pointer/finger across both layouts. */}
          <DragOverlay>
            {drag.activeEntry ? (
              <EntryChip entry={drag.activeEntry} card={drag.activeCard} variant="row" />
            ) : null}
          </DragOverlay>
        </DndContext>
      )}
...
/**
 * S25 — a single `md:+` week grid column, made a droppable target keyed by
 * its `date`. Extracted from the WeekView map because `useDroppable` is a
 * hook and can't be called inside `.map()`. The column keeps its existing
 * day-click create/delete affordance (S05/S06); the `isOver` highlight reads
 * over the chip stack on light/dark. The inner chip list stays
 * `overflow-y-auto` so dnd-kit's built-in auto-scroll can engage when a chip
 * is dragged toward a scrolled column's edge (Task 13).
 */
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // S25 — drop-target highlight while a chip hovers this column.
