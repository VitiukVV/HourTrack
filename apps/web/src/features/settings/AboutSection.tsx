 ...
 * `define` so the value flips on every release) and reserves a slot for
 * "Granted scopes" that S09 will populate once GIS PKCE lands.
 *
 * `__APP_VERSION__` is declared as a global string in `vite-env.d.ts`. In
 * tests the value is undefined; we fall back to `'dev'` so component snapshots
 * stay readable.
 */
    ...
    <SettingsSection title={t('settings.about.title')} testId="settings-about">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <span className="text-muted-foreground text-sm">
          {t('settings.about.scopesComingSoon')}
        </span>
      </div>
    </SettingsSection>
  );
