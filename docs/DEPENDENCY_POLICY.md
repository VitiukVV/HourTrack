...
- **`@dnd-kit/core` + `@dnd-kit/utilities` + `@dnd-kit/sortable` are EXACT-pinned** (no
  caret: `6.3.1` / `3.2.2` / `10.0.0` in `apps/web/package.json`), unlike the caret ranges every
  ...
  keyboard a11y announcements) before merge. `@dnd-kit/sortable` joined the pin when the
  card row gained press-and-hold reorder (2026-09-10): it peers `@dnd-kit/core@^6.3.0`, so a
  core bump can silently pull an untested sortable pairing, and the card row is now one of
  the surfaces to smoke-test alongside the calendar ones.
