...
- **[pre-existing, NOT S32]** `e2e/08-drag-reschedule.spec.ts` › "drag a chip onto another day moves the entry and persists across reload" FAILS (chip never appears on the target cell). Verified against the stashed tree. Joins the already-recorded `05-a11y` Home contrast failure as a known-red e2e case. Both still red at the end of S32.
