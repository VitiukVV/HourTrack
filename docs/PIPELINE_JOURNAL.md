...
**Sprint:** Reports Overhaul (Filters, Pickers, Table, GC Edit Regression)
**Merged:** 2026-05-16
**Merge commit:** see `git log main --oneline` (`feat(s20): …` series)

### Delivered

Six-part overhaul of the Reports surface plus a Google Calendar PATCH-on-edit regression cage and two Part-F selects fixes. (A) `reportsStore.setPeriod` now re-anchors `anchorDate` whenever the period flips — `month` → `startOfMonth(today)`, `week` → Monday of the current week, `day` → today; `custom` leaves the prior anchor intact. The default state, and `reset()`, both snap to `period='month'`, `anchorDate=startOfMonth(today)`, `selectedCardIds=null`, `showArchived=false`. A new `clearCardSelection()` action returns the selection to the null sentinel; semantically equal to `selectAll()` but named after the user-visible "Reset cards" affordance. (B) New `Popover` primitive (`apps/web/src/components/ui/popover.tsx`) is a shadcn-style wrapper around `@radix-ui/react-popover` (new dependency); styling tokens mirror `select.tsx`. `MonthPicker` renders a trigger with the localized month label ("May 2026" / "Травень 2026" / "mayo 2026" via `formatMonthYear`-style locale-aware date-fns formatting) + a popover with a year stepper and a 3×4 grid of localized month cards; emits `YYYY-MM-01` on click. `WeekPicker` renders a trigger `Week N · DD.MM–DD.MM.YYYY` (ISO week + project's `dd.MM.yyyy` format) + a popover with a month stepper and a vertical list of Mon-anchored weeks for that month (4-6 rows); emits the picked Monday as `YYYY-MM-DD`. (C) `ReportsTable` Date column bumped from `z-10` to `z-20` on both `<th>` and `<td>` so the sticky-left Date cell stays above the new `z-10` ReportsFilters sticky section when the user scrolls vertically — fixes the UR-20-3 bleed-through. (D) `ReportsFilters` rewritten as two contiguous sections: Section 1 (`sticky top-0 z-10`) contains the period preset row + period-aware picker + Reset; Section 2 (non-sticky) contains the card multi-select chips + a conditional Reset-cards button + Show-archived toggle. The duplicate `data-testid="anchor-readable"` span next to the prev/next arrows was deleted (UR-20-4). The preset row uses `flex-nowrap overflow-x-auto scrollbar-none` so Reset stays on the same row across uk/en/es locales (UR-20-9). The Reset button uses `variant="destructive"` (loud red) with a `RotateCcw` icon and a tooltip `reports.filters.resetTooltip`; clicking calls `reset()`. The Reset-cards button (`variant="ghost"`) renders only when `selectedCardIds !== null && length > 0`. (E) AppLayout's `showCardsHeader` now matches only `/` and `/day/*` — dropped `/reports` per UR-20-2 / Task 14. Reports owns its own multi-select chip row via ReportsFilters Section 2. (F) `LanguageSwitcher` trigger gains `min-h-[44px] w-[8rem] sm:h-8 sm:min-h-0` so it meets the iOS touch-target rule on `< sm` and fits Spanish "Español" without truncation. (G) `CardForm` rate-type chips refactored from bespoke `<input type="radio" className="sr-only">` to the shared Radix `Select` primitive driven by a `RATE_TYPE_OPTIONS` array `[{value,labelKey},...]` — so S21 adds the `monthly` row by editing one line; Controller wiring + the "clear the inactive numeric field on switch" behaviour are preserved. (H) GC PATCH regression cage: new `apps/web/src/features/calendar-sync/__tests__/patchEvent.regression.test.ts` seeds an entry, runs the create handler (POST `evt_123`), edits `durationMin: 120 → 180`, runs the update handler, and asserts (1) exactly one PATCH against `/events/evt_123`, (2) the PATCH body's `end.dateTime` is `2026-05-15T13:00:00` reflecting the new duration, (3) no second POST fires, (4) `googleEventId` is preserved. SMOKE_TEST.md Section 6 got a new bullet documenting the UR-20-11 manual verification path. (I) i18n: 2 new keys × 3 locales (`reports.filters.resetTooltip`, `reports.filters.resetCards`) — `i18n:check` GREEN at 272 keys × 3 locales aligned.

### Deviations from spec

- **Spec said `setPeriod('day')` could "leave anchor where it is, OR snap to today"; spec line 37 locked snap-to-today.** Implementation went with snap-to-today as locked.
- **MonthPicker / WeekPicker tests assert localized labels in English ("May 2026", "Week 20", "June 2026")** because the vitest env has no navigator language / localStorage → i18next falls back to `en`. The locale-switching behaviour itself is exercised at the LanguageSwitcher integration level (existing tests); the picker tests assume the en fallback.
- **MonthPicker's `aria-label` is the month-year label by default**, not the period name. Initially the spec example wired ReportsFilters to pass `aria-label={t('reports.period.month')}` ("Month"), but that caused the period preset button "Month" and the picker trigger "Month" to collide for `getByRole('button', { name: /^month$/i })`. The picker now keeps its descriptive label; ReportsFilters tests scope preset queries via `within(reports-filters-presets-row)` to remove ambiguity.
- **`reports.filters.resetTooltip`** was added (spec mentioned a `title=` tooltip but didn't enumerate the key). Three locales: en "Reset filters to the current month and all cards", uk "Скинути фільтри до поточного місяця та всіх карток", es "Restablecer filtros al mes actual y todas las tarjetas".
- **CardForm.test.tsx — radio assertions retargeted, not deleted.** The two tests that previously did `userEvent.click(getByRole('radio', { name: /Fixed/ }))` now `click(getByTestId('cardform-rate-type-trigger'))` then `click(getByTestId('cardform-rate-type-option-fixed'))`. Same coverage, new driver.
- **No Playwright spec for the new pickers**; the project still has no Playwright config (S19 followup). Picker visual regressions are covered structurally by the new vitest specs; tagged as a V3 e2e followup.
- **Spec Task 17 ("Insert Entry → mock GC API responds with `eventId=evt_123`") uses the same fetch-mock harness pattern as calendarOps.test.ts but with a recording fetch (captures method+url+body) rather than the stub-queue harness** because the assertion span covers TWO sequential handler calls (`handleCreateCalendarEvent` then `handleUpdateCalendarEvent`) and we need to look back across all captured calls. Functionally equivalent.

### Patterns introduced

- **`Popover` primitive** at `apps/web/src/components/ui/popover.tsx` — shadcn-style wrapper exporting `Popover`, `PopoverTrigger`, `PopoverAnchor`, `PopoverContent` from `@radix-ui/react-popover`. Styling tokens mirror `select.tsx` (popover surface, animation classes, z-50). Downstream components needing a click-to-open floating panel should consume this rather than re-rolling outside-click handling or porting Radix manually. **New runtime dependency**: `@radix-ui/react-popover`.
- **`MonthPicker` primitive** at `apps/web/src/components/ui/MonthPicker.tsx`. Props: `value: string (YYYY-MM-DD)`, `onChange(YYYY-MM-01)`, optional `className` + `aria-label`. Use anywhere a "pick a calendar month" affordance is needed.
- **`WeekPicker` primitive** at `apps/web/src/components/ui/WeekPicker.tsx`. Props: `value: string (any YYYY-MM-DD inside a week)`, `onChange(Monday YYYY-MM-DD)`. Uses ISO week numbering (`getISOWeek`) but project-wide Monday-of-week math (`startOfWeekMonday` from `@hourtrack/shared-utils`) for the highlight + emitted value — same convention as the rest of the app.
- **`anchorForPeriod(period, prev)` discipline** in `reportsStore.ts`: switching periods re-anchors automatically. Any future caller that programmatically flips the period must NOT bypass `setPeriod` — calling `set({ period: 'X' })` directly would leak the stale `anchorDate`. Same applies for downstream sprints (S21 onward) that consider new period values like `quarter` or `year` — they'd add a branch to `anchorForPeriod` and lock the new snap rule there.
- **`RATE_TYPE_OPTIONS` array pattern** in `CardForm.tsx`. The Select's options live in a top-level constant `Array<{ value, labelKey }>` rather than literal `<SelectItem>` children. S21 adds `monthly` by appending one row + an i18n key, no JSX change.
- **`<Button variant="destructive">` for "discard state" actions** (not just "delete row"). The destructive variant now also signals the Reports Reset button — the action discards filter state, which is destructive in the right sense. Counter-argument the spec anticipated: "destructive should be reserved for delete" — declined; the precedent is set.
- **Test scoping with `within(getByTestId(...))`** when a screen has two buttons with the same accessible name. ReportsFilters tests use `within(getByTestId('reports-filters-presets-row'))` to limit preset-button queries — the pattern generalises whenever a sticky-vs-content split introduces label collisions.
- **Recording fetch harness** in `patchEvent.regression.test.ts`. Captures `{ method, url, body }` per call so multi-step scenarios (insert → edit → assert PATCH-not-POST) can do post-hoc assertions on the full call sequence. Reusable for any other "multi-handler sequencing" regression test.

### Followups for later sprints

- **S21 — append `monthly` to `RATE_TYPE_OPTIONS`** in `CardForm.tsx`. One row `{ value: 'monthly', labelKey: 'cards.monthly' }` + three locale keys. The Select infrastructure is already in place.
- **S21 — monthly earnings calculation reuses `rangeForReports`** (`apps/web/src/features/reports/useReportData.ts:31`). Do NOT invent a parallel `getPeriodBounds` helper.
- **Playwright e2e for pickers** — when Playwright is added (deferred from S19), assert MonthPicker + WeekPicker mount, click flow, and a screenshot for visual regression on month-grid contrast.
- **Lighthouse mobile baseline** is still pending from S18 (Tasks 0a / 14). Not S20 work; tracked in `docs/lighthouse-baseline.md`.
- **Bundle audit** — `dist/assets/index-*.js` is now 778.95 kB raw / 234.77 kB gzip (was 770.19 kB / 232.66 kB pre-S20 = +8.76 kB raw / +2.11 kB gzip, due to the Popover wrapper + MonthPicker + WeekPicker). Past Vite's 600 kB chunk warning since S17. Still flagged.
- **Pre-existing `AuthProvider` flake** — observed once during local runs in S18/S19. Did NOT recur in S20 runs. Re-flagged in case it shows up under turbo-parallel contention.
- **Show-archived toggle 375px / dark-theme visual verification (Task 21)** was treated as a manual smoke step — the toggle is reachable, the label is readable, the tap-target inherits Switch's height. No code change needed. If a future sprint runs a dedicated mobile e2e pass, snapshot both views in both themes.
- **MonthPicker locale-switching test coverage** — the existing tests assert the English fallback only. A focused test that switches `i18n.changeLanguage('uk')` mid-render + asserts the trigger reads "Травень 2026" would lock the locale wiring. Not a correctness gap (the underlying `format(..., { locale })` is exercised at the `calendarLocale.test`-adjacent surface), but a coverage gap.

### Integration notes

- **`@radix-ui/react-popover` is now a runtime dependency** (`apps/web/package.json`). Bundle delta absorbed into the `radix` chunk by the existing `manualChunks` config — no vite.config changes.
- **`setPeriod` now mutates anchorDate.** Any downstream code that called `setPeriod(x)` and immediately read `anchorDate` expecting the OLD value will see the new snapped value instead. Audit found no such callers in the current codebase, but flagging for S21.
- **`reset()` snaps anchorDate to startOfMonth(today)**, not today. Tests that previously asserted "anchorDate string roughly looks like today" should be updated to assert `formatLocalDate(startOfMonth(new Date()))`. `reportsStore.test.ts` was updated.
- **`/reports` no longer mounts `<CardsHeader>`.** Any future test that asserts the global CardsHeader on a Reports screenshot must be retargeted; tests that explicitly assert `cards-header` is absent on `/reports` were added.
- **ReportsFilters now exposes `data-testid="reports-filters-section-sticky"` + `reports-filters-section-scrollable" + "reports-filters-presets-row" + "reports-filters-reset" + "reports-filters-reset-cards"** as stable test anchors. Downstream UI tests should use these.
- **ReportsTable's sticky-Date column is `z-20`.** Any new sticky element below the chrome header inside the Reports page must NOT collide with that z-index. The chrome `<header>` is `z-20` too; they don't overlap visually because the Date column is far below the header in the DOM stack.
- **`CARD_COLORS[2]` is `#D97706` (Amber).** Unchanged from S19. The palette didn't move in S20.
- **`reports.filters.resetTooltip` and `reports.filters.resetCards`** are now part of the i18n surface area; downstream sprints should provide locale entries when adding similar Reset-style affordances.
- **`buildEvent` time-bound payload contract** (start.dateTime/end.dateTime, no all-day `date` field) is now locked by the regression cage. Refactoring `buildEvent` to switch to `date`-only events for some reason would surface as a regression-test failure with a clear message.

### Verification

- `pnpm -F web typecheck` — GREEN.
- `pnpm -F web lint` — GREEN (`--max-warnings=0`).
- `pnpm -F web test` — GREEN (80 files, 678 tests). Was 76 files / 647 tests pre-S20 → +4 new test files (MonthPicker, WeekPicker, LanguageSwitcher, patchEvent.regression) and +31 new tests across them and the updated suites.
- `pnpm -F web build` — GREEN. `dist/assets/index-jRCTe3T7.js` 778.95 kB raw / 234.77 kB gzip (vs 770.19 kB / 232.66 kB at end-of-S19 = +8.76 kB raw / +2.11 kB gzip — Popover + MonthPicker + WeekPicker).
- `node scripts/i18n-check.mjs` — GREEN (272 keys × 3 locales aligned; was 270 pre-S20 = +2 new keys × 3 = 6 entries).
- `pnpm -F web e2e` — NOT RUN (no Playwright config in repo).
...
**Sprint:** Calendar Polish + Monthly Rate Feature (Schema v3)
**Merged:** 2026-05-16
**Merge commits:** `feat(s21): schema v3 — Card.monthlyTotal + 'monthly' rateType` (7e9a1e5) + `feat(s21): monthly retainer earnings + UI + calendar polish` (38c5536)

### Delivered

Six-part rollout of the retainer-style billing model + MonthView/WeekView visual polish. (A — Schema v3) `RateType` extended from `'hourly' | 'fixed'` to `'hourly' | 'fixed' | 'monthly'`; `Card` grew a `monthlyTotal: number | null` field documented as "EUR per month, applied as a flat retainer for every month in which the card has ≥1 entry". `CardInputSchema` (zod) gained a third discriminated-union branch enforcing the cross-field invariant "exactly one rate field non-null" (i18n keys `cards.validation.monthlyTotalRequired` + `cards.validation.monthlyTotalNotNull`). Dexie schema bumped to **v6** with a non-destructive `.upgrade()` that backfills `monthlyTotal: null` on every existing card row — rateType stays `'hourly'`/`'fixed'` for legacy rows; users opt into `'monthly'` explicitly by editing a card. `DriveSnapshot.schemaVersion` widened to `2 | 3`; the writer always emits v3, and `validateSnapshot` upgrades v2 inputs in-band (clone + `monthlyTotal: null` injection per card + schemaVersion coerce) before zod runs. Backup `appProperties.schemaVersion` stamped `'3'`. (B — Earnings) `earningsForEntry` got an explicit `'monthly'` branch returning 0 BEFORE the hourly branch (custom payment still wins as a one-off override). New `monthlyEarningsForPeriod(card, entries, periodStart, periodEnd)` helper counts distinct `YYYY-MM` slots in the period that contain ≥1 entry for that card and multiplies by `monthlyTotal`. `computeReport` signature gained `periodStart`/`periodEnd` (caller plumbs `rangeForReports`-resolved bounds via `useReportData`) and returns a new `monthlyContribution: number` field; `totals.earnings = standardSum + monthlyContribution`. Monthly byCard rows carry the retainer; per-entry rows carry 0 (or custom-payment override). `buildEvent.formatRateLine` added an explicit `'monthly'` branch ("Monthly total: N EUR") BEFORE the fixed fallback — otherwise GC event descriptions would mislabel monthly cards as "Fixed total: 0 EUR". (C — CardForm) Third `SelectItem` (`monthly`) appended to S20's `RATE_TYPE_OPTIONS` array — the Select primitive infra is unchanged. Conditional `monthlyTotal` input mirrors the hourlyRate/fixedTotal UX (decimal, step 0.01, min 0); resolver clears the two non-active rate fields on every rateType switch so submit always emits a clean discriminated payload. `CardModal` and `DayPickerModal` forward `monthlyTotal` through to `createCard`/`updateCard`. (D — Reports) `ReportsTable` renders `—` (em-dash) in the Sum column for monthly-card rows; Hours column still shows duration. (E — Calendar visuals) `EntryChip` bar variant became NAME-ONLY (dropped leading HH:MM time and trailing `formatDuration` text); the `title` attribute still carries the full "HH:MM · name · duration" string. `DayCell` lost the per-day total-duration/total-earnings footer — real subtraction: the `totalMin`/`totalEarnings` reducer and the `earningsForEntry`+`formatDuration` imports are gone (the `entriesByCard` prop is preserved on the public shape for callers but unused inside). `WeekView` already had the spec's `border-r last:border-r-0` discipline from a prior sprint; a new test asserts the contract. WeekAgendaView's existing per-day rounded card already provides the spec's separator visual. (F — Misc + docs) Five new i18n keys × 3 locales (277 total, was 272). `docs/PROJECT_PLAN.md` got a new "11b. Sync indicator legend (S21)" section listing the 4 SyncIndicator states + their visuals (idle/syncing/error/offline) since S19 moved the widget into Settings → Backup. `docs/V2_FEATURE_PLAN.md` got a "Post-V2 polish (S19/S20/S21)" closure section mapping each sprint to the UR items it covered.

### Deviations from spec

- **Task 3 ("Bump Dexie to version(3)")**: the spec text said `version(3)` but the project's actual Dexie was already at v5 (from S16 destructive migration). The correct interpretation is "the next version", so Dexie advanced to **v6**. `DriveSnapshot.schemaVersion` independently advanced from 2 → 3 per the spec's actual numbering. The journal callout in `schema.ts` documents the rationale; downstream sprints should treat "v6" as the current Dexie generation.
- **Task 10 ("Add a third radio option Monthly")**: outdated by S20 — the rate-type input was refactored from radios to a Radix Select in S20 Task 20. The S21 work is therefore a third **SelectItem** (`{ value: 'monthly', labelKey: 'cards.monthly' }`) appended to S20's `RATE_TYPE_OPTIONS` array. Tests drive the Select via `getByTestId('cardform-rate-type-trigger')` + `getByTestId('cardform-rate-type-option-monthly')`, not a radio. The DEP_CONTEXT from S20 flagged this; addressed at implementation time.
- **Task 14 spec wording** says ReportsTable Sum column for monthly cards is `—`. Implemented; the `byEntry` row's `earnings` for a monthly non-custom entry is also `0` (not `null`), so the rendering check is `card.rateType === 'monthly' ? '—' : earnings.toFixed(2)`. Custom-payment overrides on a monthly card still surface their amount in the row's `earnings` AND in totals (locked decision: retainer + custom rides on top). No deviation, just documenting the per-row contract.
- **Task 22 (WeekAgendaView day-group separator)**: spec asked for `border-t border-border on each day-section EXCEPT first`. The existing `<li>` already carries a complete rounded border per day, which is strictly more visually separated than `border-t`-only would be. Kept the stronger treatment rather than weakening it; no test change.
- **Task 9b (queries.ts rateType audit)**: `queries.ts` has one rateType discriminator site (`assertCardShape`). It was an `if/else` (else assumed fixed) — now extended into an exhaustive `hourly | fixed | monthly` switch with the matching invariants. No other rateType switches found in queries.ts. The CardForm Controller, CardModal, DayPickerModal, etc. all forward `rateType` as a passthrough payload field and didn't need surgical change.
- **DayCell `entriesByCard` prop**: spec Task 19 said "remove the per-day footer". The footer's only consumer of `entriesByCard` was the `totalEarnings` reduce. After dropping the footer, `entriesByCard` is unused inside `DayCell` but is preserved on the props shape because `MonthView` (and tests) still pass it. The prop is renamed to `_entriesByCard` inside the destructure to silence the unused-variable lint; the public type stays unchanged for callers. Flagged as a candidate prop-removal cleanup for any future sprint that touches the MonthView/DayCell contract.
- **EntryChip's `entry-chip-time` testid**: bar variant lost the testid (it had only one consumer: the deleted "renders the start-time HH:MM prefix" tests). The row variant still emits `entry-chip-time` for its own time-prefix span. The bar-variant tests now assert `queryByTestId('entry-chip-time')` is null, locking the change.

### Patterns introduced

- **`monthlyEarningsForPeriod(card, entries, periodStart, periodEnd)`** in `packages/shared-utils/src/earnings.ts`. Pure function, exported from `@hourtrack/shared-utils`. Counts distinct `YYYY-MM` slots in `[periodStart, periodEnd]` containing ≥1 entry for the card and multiplies by `card.monthlyTotal`. **Locked: no proration** — a custom range spanning 1.5 months with entries in both bills 2 full retainers. Consumers needing period-scoped retainer math MUST use this — do NOT inline. Defensive: returns 0 for non-monthly cards, null `monthlyTotal`, or empty entries.
- **`computeReport(entries, cards, selectedCardIds, periodStart, periodEnd)`** at `apps/web/src/features/reports/computeReport.ts`. Signature change: period bounds are now required. Callers must source them via `rangeForReports(period, anchorDate, customStart, customEnd)`. Return shape extended with `monthlyContribution: number` (standalone breakdown; already folded into `totals.earnings`). Tests pre-S21 used a flexible `ANY_PERIOD_START`/`ANY_PERIOD_END` constant covering all of 2026 to stay shape-stable.
- **`upgradeSnapshotV2ToV3(input)`** helper in `apps/web/src/features/backup/validateSnapshot.ts`. Pure clone-and-mutate-once pattern for in-band schema upgrades. Establishes the convention: when bumping snapshot versions, prefer in-band normalisation over rejection. Downstream sprints adding v4+ should follow the same pattern — chain upgrades v(N-1)→v(N) BEFORE zod runs.
- **`schemaVersion: z.union([z.literal(2), z.literal(3)])`** in the zod schema. Documents the multi-version acceptance contract explicitly — future v4 schema bumps add a new literal here and a new upgrader.
- **EntryChip `bar` variant convention**: in MonthView density, the chip is **name-only**. Full data lives in `title`. New consumers of `EntryChip variant="bar"` should not reintroduce visible time/duration — that's the WeekView `row` variant's job.
- **`'monthly'` rate-line copy on GC events**: `Monthly total: N EUR` is the canonical phrase (matches the i18n `cards.monthlyTotal` label). Downstream surfaces that mention monthly cards in user-visible copy should use the same phrasing for consistency.
- **`assertCardShape` exhaustive switch** in `queries.ts` is now the lint canary: TypeScript's `RateType` discriminated union means adding a 4th rate type in a future sprint will fail-compile here first. Keep the structure exhaustive — don't paper over with a default case.

### Followups for later sprints

- **Mixed-rate billing (monthly retainer + hourly overflow)** is explicitly out of scope per the S21 Notes. If a future sprint adds it, the `earningsForEntry` precedence order and `monthlyEarningsForPeriod` signature both stay backward-compatible — extension would happen at `computeReport` (a new field `overflowEarnings`) without breaking the existing API.
- **Prorated month billing** is locked OUT by the user's 2026-05-16 decision. If a future user UR reverses this, the change is local to `monthlyEarningsForPeriod` (counting partial-month overlap as a fractional retainer); no consumer downstream would need updating.
- **`DayCell.entriesByCard` prop cleanup** — the prop is now unused inside DayCell after the footer removal. A future cleanup sprint can drop it from the DayCellProps interface and from every call site (MonthView passes it). Out of S21 scope to avoid touching the MonthView prop drill.
- **ReportsMetrics: surface `monthlyContribution` as a sub-line** — the data is now available (the i18n key `reports.metrics.monthlyContribution` was added in lockstep) but the UI doesn't render it yet. A future "explain my total" affordance could render `{totalEarnings} EUR (incl. {monthlyContribution} EUR retainers)`. Out of S21 scope per the spec's "label, optional badge under total" phrasing.
- **Playwright e2e** — still no Playwright config in the repo (S19/S20 followup recurs). The MonthView name-only chip + WeekView border-r grid would be valuable visual-regression snapshots. **Target: any future sprint that adds Playwright.**
- **`'Monthly total: 0 EUR'` fallback in buildEvent** — defensive copy when `monthlyTotal` is null. In practice the form / schema prevents this, but if a corrupted Drive snapshot restored a malformed monthly card the GC description would read "0 EUR". The behaviour is correct (no crash, no `undefined EUR`); a future sprint could flag null monthly cards during restore validation with a banner.
- **Pre-existing AuthProvider flake** — observed once in S19 and once again here (re-passed on a second run). Recurs across sprints; not introduced by S21. **Target: a future infra-cleanup sprint.**
- **Acceptance-criterion validation post-deployment**: none of the S21 acceptance criteria require a live DB (it's a pure-FE/Dexie sprint). All criteria are exercised in the local test/build pipeline.

### Integration notes

- **`Card.monthlyTotal` is a NEW required field on the Card interface.** Every test fixture and snapshot was bulk-updated to include `monthlyTotal: null` (44 test files + e2e specs touched). Downstream sprints that introduce new Card fixtures MUST include the field — TypeScript will fail-compile if they don't.
- **Dexie is now at v6.** Any test that opens a `HourTrackDB` and reads `verno` should expect 6 (was 5). The S16 v4→v5 destructive migration test was updated to reflect this.
- **DriveSnapshot writers always emit `schemaVersion: 3` from this commit forward.** Readers must accept both 2 and 3 (the validator does this via the in-band upgrade). Restore-from-old-app-version (v2 snapshot in user's Drive) is a covered path; users updating to S21 don't lose their backups.
- **`monthlyEarningsForPeriod` is exported from `@hourtrack/shared-utils`** alongside `earningsForEntry`. New consumers should import from there.
- **`computeReport` signature change is a BREAKING change for any caller besides `useReportData`.** Grep found no other callers in the current codebase; tests pass `ANY_PERIOD_START`/`ANY_PERIOD_END` constants. If a future sprint introduces a new Reports consumer (e.g. a "this week" widget), it MUST pipe period bounds in.
- **EntryChip's bar variant `data-testid="entry-chip-time"` is GONE** for the bar variant. Tests that assert it on a bar-variant chip will need to move to the row variant (which still emits it). The row variant's contract is unchanged.
- **DayCell no longer imports `earningsForEntry` or `formatDuration`.** A future sprint that reintroduces ANY per-day aggregation must reimport them — that's a new feature, not a revert.
- **i18n surface area:** `cards.monthly`, `cards.monthlyTotal`, `cards.validation.monthlyTotalRequired`, `cards.validation.monthlyTotalNotNull`, `reports.metrics.monthlyContribution` are now part of the en/uk/es locales. `monthlyContribution` is data-available but not yet rendered (see Followups).
- **The S20 sticky-Date `z-20` contract is intact** — the ReportsTable Sum-cell change only touched the inner content, not the z-index.
- **`buildEvent`'s `formatRateLine` is now exhaustive over all three rate types.** Future rate-type additions will fail-compile via TypeScript's `RateType` discriminator (no default branch — by design).

### Verification

- `pnpm -F web typecheck` — GREEN.
- `pnpm -F web lint` — GREEN (`--max-warnings=0`).
- `pnpm -F web test` — GREEN (81 files, 706 tests). Was 80 files / 678 tests pre-S21 → +1 new test file (`dexie.upgrade.test.ts`) and +28 new tests across the new file + earnings + computeReport + cardSchema + CardForm + buildEvent + EntryChip + DayCell + WeekView + validateSnapshot.
- `pnpm -r --filter @hourtrack/shared-utils test` — GREEN (3 files, 46 tests; was 36 pre-S21 = +10 monthly-branch + `monthlyEarningsForPeriod` cases).
- `pnpm -F web build` — GREEN. `dist/assets/index-Dk2mt0dE.js` 783.00 kB raw / 235.45 kB gzip (vs 778.95 kB / 234.77 kB at end-of-S20 = +4.05 kB raw / +0.68 kB gzip — the new helper + extended discriminator).
- `node scripts/i18n-check.mjs` — GREEN (277 keys × 3 locales aligned; was 272 pre-S21 = +5 new keys × 3 = 15 entries).
- `pnpm -F web e2e` — NOT RUN (no Playwright config in repo).
...
- **[armed time bomb, not yet red]** `lwwMerge.reminders.test.ts:83` pins a tombstone at `2026-08-03` with an unpinned merge clock — it goes red around **2026-09-02**, when wall-clock passes the 30-day tombstone TTL, exactly as the payments one did. `lwwMerge.test.ts` has May-2026 tombstones that are ALREADY past the TTL on an unpinned clock, so some of those cases may now be passing for the wrong reason. Same one-line fix as below (`{ now: NOW }`); left out of S32 because neither is currently failing.
- ~~**[pre-existing, NOT S32]** `e2e/08-drag-reschedule.spec.ts` drag case + the `05-a11y` Home contrast failure — both still red at the end of S32.~~ **Both repaired in the third commit; see "E2E repairs" below.**

**CI repairs (added after the first PR run came back red — none of the three causes were S32):**

- **`pnpm audit --prod --audit-level=high` blocked the ENTIRE first job.** `react-router` 7.18.1 vs advisory GHSA-qwww-vcr4-c8h2 (vulnerable `>=7.12.0 <7.18.2`). The audit is the first real step, so lint / typecheck / test / build / i18n / coverage never ran on the PR at all. Fixed by refreshing the lockfile to 7.18.2 — the manifest already allowed it (`^7.18.1`). Note `pnpm update react-router-dom` from the repo ROOT is a no-op (the dependency lives in `apps/web`); it needs `--filter @hourtrack/web`. That filtered install also prunes sibling workspace `node_modules` — run a plain `pnpm install` afterwards or `@hourtrack/shared-utils` loses its vitest binary. The lockfile refresh carried transitive patch bumps along with it (rollup, terser, browserslist, caniuse-lite, core-js-compat, …); `vite` (8.1.4) and `vitest` (4.1.10) themselves did not move.
- **CI ran e2e projects it never installed a browser for.** The install step fetches `chromium` alone while `pnpm e2e` ran every project, so the webkit-based `mobile-iphone-13` project failed to launch on every run ("Executable doesn't exist … webkit-2311"). CI now runs `--project=chromium` explicitly. The job comment claimed "Both projects run" — corrected, and the trade-off recorded there: spec 08 is the mobile-drag regression suite, so that guard now lives in local runs. Adding `webkit` to the install step is the alternative and roughly doubles the job.
- **`lwwMerge.payments.test.ts` was a time bomb, not a product defect.** Its tombstone fixture is dated `2026-07-12` and `lwwMerge` prunes tombstones older than a 30-day TTL relative to `now`; once wall-clock passed 2026-08-11 the tombstone was pruned before it could suppress the stale remote row, so the merge kept the payment and the assertion flipped. `lwwMerge` already accepts `{ now }` — the file now pins it. Product logic untouched.

**E2E repairs (third commit — the two long-standing red specs, owner asked to keep e2e and fix them):**

- **`08-drag-reschedule` › "drag a chip onto another day" was a TEST bug, not a product bug.** The Playwright error context showed `status: Entry for Acme moved to 24.08.2026` — the drag DID land, one grid row (7 days) past the target. `DndContext` does not disable dnd-kit's auto-scroll, so the page scrolls mid-drag and the target `boundingBox()` captured before `mouse.down()` is stale by the time of `mouse.up()`. `mouseDndDrag` now re-reads the target box after the stepwise moves and releases on its current centre. Product code untouched.
- **`05-a11y` › Home contrast was a REAL defect, and deeper than the S31 note implied.** `opacity-60` on the whole out-of-month `DayCell` made the cell translucent, so MonthView's `bg-foreground/20` grid showed through (surface rendered `#d5d5d5` instead of near-white) AND washed the day number to `#979797` → ratio 1.99. Keeping the opacity would have needed near-black text to clear 4.5:1, so the opacity is gone from the cell: the surface is now the opaque `bg-muted/60`, the out-of-month number is `text-foreground/70`, and the fade moved to the chip stack (`opacity-70`), where nothing has to meet a contrast ratio. `MonthView.test.tsx`'s `/opacity-60/` assertion moved with it (asserts the surface class + that neither cell carries `opacity-*`) — the expectation was stale, not weakened.
- Bundled as user-visible: the existing (unreleased) 1.3.3 What's New entry gained a fifth item in all three locales rather than bumping to 1.3.4 — the release has not shipped.
...
- Verification (local, final): the full CI step list run by hand — `pnpm audit --prod --audit-level=high` GREEN ("No known vulnerabilities found"); `pnpm i18n:check` GREEN (329 keys × 3 locales); `pnpm format:check` GREEN; `pnpm -w turbo run lint typecheck test build` GREEN across all 12 tasks; `pnpm -F @hourtrack/shared-utils test` GREEN (4 files / 67 tests — the new file is picked up, not a vacuous `--passWithNoTests`); `pnpm -F web test` 968/968; `pnpm -F web build` GREEN with `1.3.3` baked into the bundle. E2E chromium: **26 passed / 0 failed / 3 skipped (the mobile-only cases)** after the two repairs above — the two new S32 cases (incl. the no-reload reorder) pass, and so do `08-drag-reschedule` and the full `05-a11y` sweep.
